...
const STATE = { route: "home", routeArg: null, history: [], flashIndex: 0, flashSet: null, bigOIndex: 0, bigOScore: 0, bigOAnswered: false, bigOOrder: null, streak: 0 };
...
function go(route, arg) {
  ...
  STATE.route = route; STATE.routeArg = arg; render();
}
function back() {
  const prev = STATE.history.pop();
  if (prev) { STATE.route = prev.route; STATE.routeArg = prev.arg; render(); } else { go("home"); }
}
...
function daysUntil() {
  ...
  return Math.ceil((target - new Date()) / (24 * 3600 * 1000));
}
...
SCREENS.home = function(root) {
  ...
  root.appendChild(el("p", { class: "muted", style: { textAlign: "center" } }, "Built for Dr Cetinkaya's COMP5066 · Tap any tile to practise"));
};

function quickTile(icon, ttl, sub, route) {
  return el("button", { class: "quick-tile", onclick: () => go(route) },
    el("div", { class: "icon" }, icon), el("div", { class: "ttl" }, ttl), el("div", { class: "sub" }, sub));
}
...
SCREENS.lectures = function(root) {
  ...
  LECTURES.forEach(l => {
    grid.appendChild(el("button", { class: "lect-tile" + (l.examQ ? " exam" : ""), onclick: () => go("lecture", l.n) },
      el("span", { class: "num" }, "LECTURE " + l.n), el("div", { class: "name" }, l.title), el("span", { class: "tag" }, l.tag)));
  });
  root.appendChild(grid);
};
...
SCREENS.flashcards = function(root) {
  ...
  topics.forEach(tp => {
    filterRow.appendChild(el("button", { class: "chip" + (curFilter === tp ? " active" : ""), onclick: () => {
      STATE.flashFilter = tp;
      STATE.flashSet = shuffle(tp === "all" ? FLASHCARDS : FLASHCARDS.filter(c => c.topic === tp));
      STATE.flashIndex = 0; render();
  ...
  root.appendChild(el("div", { class: "flashcard-meta" }, el("span", {}, `Card ${i + 1} / ${STATE.flashSet.length}`), el("span", { class: "progress-pill" }, card.tag)));
  ...
  root.appendChild(el("p", { class: "muted", style: { textAlign: "center", marginTop: "12px" } }, "Q1 of the exam is 4 × 5-mark concept questions like these."));
};
...
SCREENS.bigO = function(root) {
  ...
  root.appendChild(el("div", { class: "quiz-meta" }, el("span", {}, `Question ${STATE.bigOIndex + 1} / ${STATE.bigOOrder.length}`), el("span", { class: "tag-pill green" }, `Score: ${STATE.bigOScore}`)));
  ...
  q.options.forEach((opt, idx) => {
    const btn = el("button", { class: "quiz-opt", onclick: () => {
      ...
      if (idx === q.answer) { btn.classList.add("correct"); STATE.bigOScore++; toast("✓ Correct"); }
      else { btn.classList.add("wrong"); opts.children[q.answer].classList.add("correct"); toast("✗ Not quite"); }
      root.appendChild(el("div", { class: "card", style: { marginTop: "12px" } }, el("h3", {}, idx === q.answer ? "Right!" : "Explanation"), el("p", {}, q.why)));
      const next = el("button", { class: "btn", style: { marginTop: "10px", width: "100%" }, onclick: () => { STATE.bigOIndex++; STATE.bigOAnswered = false; render(); } }, STATE.bigOIndex + 1 >= STATE.bigOOrder.length ? "See results" : "Next question →");
...
function renderBigOResult(root) {
  ...
  root.appendChild(el("section", { class: "hero" }, el("h2", {}, "🎉 Quiz complete"), el("p", {}, `You scored ${score} / ${total} (${pct}%)`)));
...
SCREENS.sort = function(root) {
  ...
  root.appendChild(el("div", { class: "btn-row" },
    ...
    el("button", { class: "btn", onclick: () => { if (s.stepIdx < s.steps.length - 1) { s.stepIdx++; render(); } else toast("Sorted!"); } }, "Step →"),
...
const SORTERS = {
  ...
  selection(A) {
    ...
    for (let i = 0; i < n - 1; i++) {
      ...
      if (min !== i) { [A[i], A[min]] = [A[min], A[i]]; steps.push({ array: A.slice(), desc: `Smallest = A[${min}] = ${A[i]}. Swap with A[${i}].`, activeIdx: [i, min], sortedIdx: Array.from({length: i+1}, (_, k) => k) }); }
      else { steps.push({ array: A.slice(), desc: `A[${i}] was already smallest — no swap.`, activeIdx: [i], sortedIdx: Array.from({length: i+1}, (_, k) => k) }); }
    }
    steps.push({ array: A.slice(), desc: "✓ Array fully sorted.", sortedIdx: A.map((_, i) => i) });
    return steps;
  },
...
const COMPLEXITY = {
  insertion: { name: "Insertion sort", html: "<strong>Best:</strong> O(n) — array already sorted, inner while never runs.<br><strong>Avg / Worst:</strong> O(n²) — reverse-sorted shifts everything every pass.<br><strong>Space:</strong> O(1) — in-place.<br><strong>Stable:</strong> yes." },
  bubble:    { name: "Bubble sort",    html: "<strong>Best:</strong> O(n) — single pass with no swaps detects sorted input.<br><strong>Avg / Worst:</strong> O(n²).<br><strong>Space:</strong> O(1) — in-place.<br><strong>Stable:</strong> yes." },
  selection: { name: "Selection sort", html: "<strong>All cases:</strong> O(n²) — inner loop runs regardless.<br><strong>Space:</strong> O(1) — in-place.<br><strong>Stable:</strong> typically no." },
  merge:     { name: "Merge sort",     html: "<strong>All cases:</strong> O(n log n) — recurrence T(n) = 2T(n/2) + O(n).<br><strong>Space:</strong> O(n) — auxiliary arrays for merging.<br><strong>Stable:</strong> yes — preferred when stability + worst-case guarantee matter." }
};
...
SCREENS.bst = function(root) {
  ...
  root.appendChild(el("section", { class: "card" },
    ...
    el("div", { class: "btn-row", style: { marginTop: "10px" } },
      ...
      el("button", { class: "btn btn-secondary", onclick: () => {
        ...
        s.values = vals; s.input = vals.join(", "); s.preset = "Random"; s.presetNote = "Random practice set — try predicting balance first!"; rebuildBST(); render();
      } }, "🎲 Random"),
  ...
  if (tree) {
    ...
    root.appendChild(el("section", { class: "card" }, el("h3", {}, "Balance check"), el("p", { html:
      `<strong>Height:</strong> ${info.height}<br><strong>Node count:</strong> ${info.size}<br>` +
      `<strong>Min balanced height (⌈log₂(n+1)⌉):</strong> ${info.minH}<br>` +
      `<strong>Largest |left − right| diff:</strong> ${info.maxDiff}<br><br>` +
      (info.maxDiff <= 1 ? `<span class="tag-pill green">BALANCED</span> — every node's subtree heights differ by ≤ 1.` : `<span class="tag-pill red">UNBALANCED</span> — at least one node has subtree heights differing by ${info.maxDiff}.`)
    })));
    ...
    root.appendChild(el("section", { class: "card" }, el("h3", {}, "Traversals"), el("p", { html:
      `<strong>Inorder</strong> (L, N, R): ${inorder.join(" , ")}<br><em class="muted">Note: inorder of a BST is sorted — useful sanity check.</em><br>` +
...
function bstAnalyse(root) {
  ...
  function rec(n) { if (!n) return 0; size++; const l = rec(n.left), r = rec(n.right); maxDiff = Math.max(maxDiff, Math.abs(l - r)); return 1 + Math.max(l, r); }
  const height = rec(root);
  return { size, height, maxDiff, minH: Math.ceil(Math.log2(size + 1)) };
}

function renderBSTSVG(root, lastInserted) {
  if (!root) return el("svg", { viewBox: "0 0 300 80" }, el("text", { x: "150", y: "45", "text-anchor": "middle", fill: "#94a3b8", "font-size": "14" }, "(empty tree)"));
  ...
  layout(root, 0);
  const cols = x, colW = Math.max(50, Math.min(80, Math.floor(360 / Math.max(cols, 1)))), rowH = 60;
  ...
  nodes.forEach(n => {
    ...
    const txt = svgEl("text", { x: n.cx, y: n.cy + 4, "text-anchor": "middle", fill: n.last ? "#03261b" : "#e2e8f0", "font-size": "13", "font-weight": "700", "font-family": "system-ui" });
...
SCREENS.graph = function(root) {
  if (!STATE.graph) {
    ...
    STATE.graph = { preset: p.name, nodes: p.nodes.slice(), edges: p.edges.map(e => e.slice()), start: p.start, directed: p.directed, algo: "bfs", steps: null, stepIdx: 0, note: p.note };
    buildGraphSteps();
  }
  const g = STATE.graph;
  ...
  const presetRow = el("div", { class: "chip-row" });
  GRAPH_PRESETS.forEach(p => presetRow.appendChild(el("button", { class: "chip" + (g.preset === p.name ? " active" : ""), onclick: () => { g.preset = p.name; g.nodes = p.nodes.slice(); g.edges = p.edges.map(e => e.slice()); g.start = p.start; g.directed = p.directed; g.note = p.note; buildGraphSteps(); render(); } }, p.name)));
  root.appendChild(presetRow);
  root.appendChild(el("section", { class: "card" },
    el("div", { class: "row-between" },
      el("label", { class: "field", style: { margin: 0 } }, "Start vertex: "),
      el("select", { onchange: e => { g.start = e.target.value; buildGraphSteps(); render(); }, style: { width: "100px" } }, ...g.nodes.map(n => { const o = el("option", { value: n }, n); if (n === g.start) o.selected = true; return o; }))
    ),
  ...
  root.appendChild(el("section", { class: "card" },
    ...
    g.stepIdx === g.steps.length - 1 ? el("p", { class: "tag-pill green" }, "Complete!") : null
  ));
  root.appendChild(el("div", { class: "btn-row" },
    el("button", { class: "btn btn-secondary", onclick: () => { g.stepIdx = Math.max(0, g.stepIdx - 1); render(); } }, "← Step back"),
    el("button", { class: "btn", onclick: () => { if (g.stepIdx < g.steps.length - 1) { g.stepIdx++; render(); } else toast("Traversal complete!"); } }, "Step →"),
  ...
  root.appendChild(log);
  root.appendChild(el("p", { class: "muted", style: { textAlign: "center", marginTop: "12px" } }, "Tie-break rule: when multiple unvisited neighbours, pick the one earliest in the alphabet."));
};

function animateGraph() {
  const g = STATE.graph;
  if (g.timer) { clearInterval(g.timer); g.timer = null; return; }
  g.timer = setInterval(() => { if (g.stepIdx >= g.steps.length - 1) { clearInterval(g.timer); g.timer = null; return; } g.stepIdx++; render(); }, 700);
}

function adjacency() {
  const g = STATE.graph, adj = {};
...
function buildGraphSteps() {
  ...
  if (g.algo === "bfs") {
    ...
    steps.push({ visited: new Set(visited), frontier: queue.slice(), current: null, text: `Start: enqueue ${g.start}. Queue = [${g.start}].`, html: `Initialise with start vertex <strong>${g.start}</strong>. Queue = <code>[${g.start}]</code>.` });
    while (queue.length) {
      const v = queue.shift();
      const newOnes = adj[v].filter(u => !visited.has(u));
      newOnes.forEach(u => { visited.add(u); queue.push(u); });
      steps.push({ visited: new Set(visited), frontier: queue.slice(), current: v, justVisited: v,
    ...
    steps.push({ visited: new Set(visited), frontier: [], current: null, text: `✓ Queue empty — BFS complete.`, html: `✓ Queue empty — BFS complete. Visit order: <strong>${steps.filter(s => s.justVisited).map(s => s.justVisited).join(" → ")}</strong>.` });
  } else {
    const visited = new Set(), stack = [g.start];
    steps.push({ visited: new Set(visited), frontier: stack.slice(), current: null, text: `Start: push ${g.start}. Stack = [${g.start}].`, html: `Initialise with start vertex <strong>${g.start}</strong>. Stack = <code>[${g.start}]</code>.` });
    while (stack.length) {
      const v = stack.pop();
      ...
      steps.push({ visited: new Set(visited), frontier: stack.slice(), current: v, justVisited: v,
    ...
    steps.push({ visited: new Set(visited), frontier: [], current: null, text: `✓ Stack empty — DFS complete.`, html: `✓ Stack empty — DFS complete. Visit order: <strong>${steps.filter(s => s.justVisited).map(s => s.justVisited).join(" → ")}</strong>.` });
  }
  g.steps = steps; g.stepIdx = 0;
}

function renderGraphSVG() {
  const g = STATE.graph, W = 320, H = 280, cx = W/2, cy = H/2, R = Math.min(W,H)/2 - 30, n = g.nodes.length, pos = {};
  g.nodes.forEach((node, i) => { const angle = -Math.PI/2 + (2*Math.PI*i)/n; pos[node] = { x: cx + R*Math.cos(angle), y: cy + R*Math.sin(angle) }; });
  const step = g.steps[g.stepIdx];
  const svg = el("svg", { viewBox: `0 0 ${W} ${H}`, style: { maxHeight: "320px" } });
  const defs = svgEl("defs", {}), marker = svgEl("marker", { id: "arr", viewBox: "0 0 10 10", refX: "10", refY: "5", markerWidth: "8", markerHeight: "8", orient: "auto" });
  marker.appendChild(svgEl("path", { d: "M0,0 L10,5 L0,10 z", fill: "#94a3b8" })); defs.appendChild(marker); svg.appendChild(defs);
  g.edges.forEach(([u, v]) => {
    const a = pos[u], b = pos[v]; if (!a || !b) return;
    const dx = b.x-a.x, dy = b.y-a.y, len = Math.hypot(dx,dy), r = 18;
    svg.appendChild(svgEl("line", { x1: a.x+(dx/len)*r, y1: a.y+(dy/len)*r, x2: b.x-(dx/len)*r, y2: b.y-(dy/len)*r, stroke: "#64748b", "stroke-width": 2, "marker-end": g.directed ? "url(#arr)" : "" }));
  });
  g.nodes.forEach(node => {
    ...
    const t = svgEl("text", { x: p.x, y: p.y+5, "text-anchor": "middle", fill: textColor, "font-size": "14", "font-weight": "700", "font-family": "system-ui" });
