...
const LECTURES = [
  ...
  },
  {
    ...
    outline: ["Optimisation problems", "Approximation algorithms", "Greedy algorithms", "Knapsack", "Scheduling"],
    keypoints: [
...
const BIGO_QUIZ = [
  { code: `def f(n):\n    total = 0\n    for i in range(n):\n        total += i\n    return total`, options: ["O(1)","O(log n)","O(n)","O(n²)"], answer: 2, why: "Single loop runs n times, constant work per iteration ⇒ O(n) time, O(1) space." },
  { code: `def f(n):\n    for i in range(n):\n        for j in range(n):\n            print(i, j)`, options: ["O(n)","O(n log n)","O(n²)","O(2ⁿ)"], answer: 2, why: "Nested loops, each n iterations ⇒ n × n = n² operations." },
  { code: `def f(arr):\n    lo, hi = 0, len(arr) - 1\n    while lo <= hi:\n        mid = (lo + hi) // 2\n        if arr[mid] == target: return mid\n        if arr[mid] < target: lo = mid + 1\n        else: hi = mid - 1\n    return -1`, options: ["O(1)","O(log n)","O(n)","O(n log n)"], answer: 1, why: "Binary search halves the search range each iteration ⇒ O(log n)." },
  { code: `def merge_sort(A):\n    if len(A) <= 1: return A\n    mid = len(A) // 2\n    L = merge_sort(A[:mid])\n    R = merge_sort(A[mid:])\n    return merge(L, R)   # merge is O(n)`, options: ["O(n)","O(n log n)","O(n²)","O(2ⁿ)"], answer: 1, why: "T(n) = 2 T(n/2) + O(n) ⇒ O(n log n). Each of log n levels does linear merging." },
  { code: `def f(n):\n    if n <= 1: return 1\n    return f(n-1) + f(n-1)`, options: ["O(n)","O(n log n)","O(n²)","O(2ⁿ)"], answer: 3, why: "Two recursive calls each level ⇒ recursion tree has 2ⁿ leaves ⇒ exponential." },
  { code: `def f(arr):\n    n = len(arr)\n    for i in range(n):\n        for j in range(i+1, n):\n            if arr[i] > arr[j]:\n                arr[i], arr[j] = arr[j], arr[i]\n    return arr`, options: ["O(n)","O(n log n)","O(n²)","O(n³)"], answer: 2, why: "Outer i runs n times, inner j runs roughly n-i times — total ~n(n-1)/2 = O(n²)." },
  { code: `def f(n):\n    i = 1\n    while i < n:\n        i *= 2\n    return i`, options: ["O(1)","O(log n)","O(n)","O(n log n)"], answer: 1, why: "i doubles each step (1, 2, 4, 8, …), reaching n after log₂(n) iterations." },
  { code: `def f(n):\n    for i in range(n):\n        j = 1\n        while j < n:\n            j *= 2`, options: ["O(n)","O(n log n)","O(n²)","O(log n)"], answer: 1, why: "Outer loop O(n), inner while doubles ⇒ O(log n). Combined ⇒ O(n log n)." },
  { code: `def insertion_sort(A):\n    for i in range(1, len(A)):\n        key = A[i]\n        j = i - 1\n        while j >= 0 and A[j] > key:\n            A[j+1] = A[j]\n            j -= 1\n        A[j+1] = key`, options: ["O(n)","O(n log n)","O(n²) worst, O(n) best","O(log n)"], answer: 2, why: "Worst case (reverse-sorted) every element shifts all the way ⇒ O(n²). Best case (already sorted) inner while never runs ⇒ O(n)." },
  { code: `def f(arr, target):\n    for x in arr:\n        if x == target: return True\n    return False`, options: ["O(1)","O(log n)","O(n)","O(n²)"], answer: 2, why: "Linear search — at most n comparisons ⇒ O(n)." }
];
...
const GRAPH_PRESETS = [
  {
    ...
    edges: [["A","E"],["A","B"],["E","F"],["B","C"],["B","D"],["B","I"],["D","C"],["D","I"],["F","I"],["F","G"],["C","H"],["G","H"],["H","I"]],
    start: "A", directed: true
  },
  {
